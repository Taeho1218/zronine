 ...
 *
 * 화면 끝까지 깔리는 커버 위에 왼쪽 아래로 사진 → 이름 → 이메일 → 숫자를 얹는다.
 * 두 화면이 같은 모양을 유지하도록 markup 을 한곳에 모았고,
 * 화면마다 다른 것(환경설정·팔로우 버튼)은 topRight / besideName 으로 받는다.
 *
 * 커버 사진은 서버가 coverImageUrl 을 주면 그걸 쓰고, 없으면 assets/cover 에 넣어둔
 * 기본 사진을, 그마저 없으면 브랜드 색 그라데이션을 깐다.
 */
export default function ProfileHeader({
  ...
  const cover = profile.coverImageUrl ?? coverImage
      ...
      <div className="phead__inner">
        ...
        <div className="phead__identity">
          ...
          <div className="phead__who">
            ...
            {profile.email && <span className="phead__email">{profile.email}</span>}
